...
const Widget: React.FC<WidgetProps> = ({ open, onClose, widgetState }) => {
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      const branchesData = getBranchesSync();
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Временная реализация - синхронное получение
      const employeesData = getEmployeesSync(widgetState.selectedBranchId);
  ...
  const renderContent = () => {
    ...
    if (currentStep === 'specialist-selection') {
      return (
        <SpecialistSelection
          employees={employees}
    ...
    if (currentStep === 'appointment-confirmation') {
      const selectedBranch = getSelectedBranch();
      const selectedEmployee = getSelectedEmployee();
      return (
